...
std::string modbusExceptionMessage(uint8_t code) {
    switch(code) {
        case 0x01: return "Illegal Function (function not supported)";
        case 0x02: return "Illegal Data Address (address not valid)";
        case 0x03: return "Illegal Data Value (value out of range)";
        case 0x04: return "Slave Device Failure";
        case 0x05: return "Acknowledge (request accepted, processing delayed)";
...
bool post_json(const std::string& url, const std::string& apiKey,
    ...
    if (outFrameHex.size() < 8) return false; // too short for CRC
    ...
    if (frameBytes.size() < 4) return false;
